...
class View
{
    ...
    protected static function compileTemplate(string $content): string
    {
        ...
        $content = preg_replace_callback(
            ...
            function ($matches) {
                $view = $matches[1];
                return "<?php echo \\Support\\Core\\View::renderPartial('$view'); ?>";
            },
            $content
        );
    ...
    protected static function getViewPath(string $template): string
    {
        ...
        if ($template === 'migrations') {
            $dir = $baseDir . '/support/Core/Database/Migrations/';
        ...
        if (file_exists($frameFile)) {
            ...
            throw new \Exception("View file for template '$template' not found neither as .frame.php nor .php");
        }
    }
}